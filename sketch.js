...
function setup() {
  ...
  entity = new Entity(width / 2 + 100, height / 2, 40);
  handS = new Hand(width / 2, height / 2, 1000);
  handM = new Hand(width / 2, height / 2, 300);
  handH = new Hand(width / 2, height / 2, 200);
...
function draw() {
  ...
  preCollision = preColJudge(handS);
  if (preCollision) {
    collision = colJudge(handS);
...
function preColJudge(line) {
  // 개체와 선의 시작점 방향
  let dirEnCenterLnStart = p5.Vector.sub(
    entity.pos,
    line.start,
  );
...
// 개체와 선이 만났는지(충돌) 여부
function colJudge(line) {
  // 선 벡터(초침) 제곱
  let a = line.Ln.magSq();

  // 개체에서 선으로 향하는 방향
  let dirLnStartEnCenter = p5.Vector.sub(
    line.start,
    entity.pos,
  );
